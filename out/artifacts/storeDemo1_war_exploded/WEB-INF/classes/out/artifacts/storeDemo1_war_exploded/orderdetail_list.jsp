<%@ page import="bean.Order_Details" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function exportExcel(){
            window.location.href = "ExportOrderDetailsAction2.action?type="+3
            alert("导出成功")
        }
       /* function exportExcel(){
            if( confirm('您确定要导出吗？') ){
...
<form id="submitForm" name="submitForm" action="" method="post">
    ...
    <div class="ui_content">
        <div class="ui_tb">
            ...
            <%--表格--%>
                <%
                    List<Order_Details> list = (List<Order_Details>) session.getAttribute("OrderDetail");
                    for(int j=0;j<list.size();j++){
                %>
                <tr>
                    <td><input type="checkbox" name="IDCheck" value="14458579642011" class="acb" /></td>
                    <td><%=j+1%></td>
                    <td><%=list.get(j).getGoods()%></td>
                    <td><%=list.get(j).getOrder_number()%></td>
                    <td><%=list.get(j).getPrice()%></td>
                    <td><%=list.get(j).getNumber()%></td>
                    <td><%=list.get(j).getTotal_number()%></td>
                    <td>
