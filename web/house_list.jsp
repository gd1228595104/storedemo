...
<head>
    ...
    <script type="text/javascript">
        var dele_info = <%=(String)request.getAttribute("dele_info")%>
            if(dele_info == 'success'){
                alert("删除成功")
            }
            if(dele_info == 'faile'){
                alert("删除失败")
        ...
        /** 模糊查询来电用户  **/
        function search(){
            $("#submitForm").attr("action", "adminAllGoodsAction.action?page=" + 1).submit();
        }
        ...
        function exportExcel(){
            window.location.href = "ExportGoodsInfoAction.action?type="+1;
            alert("导出成功")
        }
        /*  function exportExcel(){
              if( confirm('您确定要导出吗？') ){
  ...
  //	 			alert(fyXqCode);
        ...
        function del(fyID){
            ...
            if(fyID == '') return;
            if(confirm("您确定要删除吗？")){
                $("#submitForm").attr("action", "DELETEAction.action?goodsid=" + fyID).submit();
            }
        }
        ...
        function batchDel(){
            ...
            $("input[name='IDCheck']:checked").each(function(index, domEle){
                bjText = $(domEle).parent("td").parent("tr").last().children("td").eq(4).prev().text();
                allIDCheck += bjText + ",";
            });
                // 			alert(bjText);
...
//                 }
            ...
            if(allIDCheck.length>0) {
                ...
                if(confirm("您确定要批量删除这些记录吗？")){
                    // 提交form
                    $("#submitForm").attr("action", "DeleteManyGoodsAction.action?manyid="+allIDCheck).submit();
                }
            }
        }
        ...
        //     $("#submitForm").attr("action", "PagesAction.action?page=" + page).submit();
        // }
        ...
        function jumpInputPage(totalPage){
            ...
            if($("#jumpNumTxt").val() != ''){
                ...
                // $("#submitForm").attr("action", "PagesAction.action?page=" + pageNum).submit();
                window.location.href="GoodsInfoPagesAction.action?page="+pageNum;
            }else{
                // “跳转页数”为空
                art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'请输入合适的页数，\n自动为您跳到首页', ok:true,});
                // $("#submitForm").attr("action", "PagesAction.action?page=" + 1).submit();
                window.location.href="GoodsInfoPagesAction.action?page="+1;
            }
        }
    </script>
    <style>
...
<script language="JavaScript">
    function fun(){
        ...
        window.location.href = "GoodsKindAction.action?goods_kind="+value;
    }
    function jumpNormalPage(page,a){
        if(page>a){
            page = a;
        }
        window.location.href="GoodsInfoPagesAction.action?page="+page;
    }

    function UpdateGoods(id) {
        window.location.href = "SelectGoodsAction.action?goodsid="+id;
    }
</script>
        ...
        <div class="ui_content">
            <div class="ui_text_indent">
                <div id="box_border">
                    ...
                    <div id="box_center">
                        商品种类ID
                        <select name="fangyuanEntity.fyXqCode" id="fyXq" class="ui_select01" onchange="fun()"> <!--onchange="getFyDhListByFyXqCode();"-->
                            <option value="">--请选择--</option>
                            <%
                                List<Goods_Kind> goods = (List<Goods_Kind>)request.getSession().getAttribute("allgoodskind");
                                for(int j = 0; j<goods.size(); j++){
                            %>
                            <option value="<%=goods.get(j).getKind_id()%>"><%=goods.get(j).getKind_id()+"("+goods.get(j).getGoods_kind()+")"%></option>
                            <%
                                }
                            %>
                    ...
                    <div id="box_bottom">
                        <input type="button" value="显示全部" class="ui_input_btn01" onclick="search();" />
            ...
            <div class="ui_tb">
                <table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
                    ...
                    <%
                        ...
                        List<Goods> list = (List<Goods>) request.getSession().getAttribute("adminallGoods");
                        int i = (page_num-1)*9;
                        for(int j = i; j<(i+9);j++){
                            if(j>=list.size()){
                                break;
                    ...
                    <tr>
                        ...
                        <td><%=j+1%></td>
                        <td><%=list.get(j).getGoods_name()%></td>
                        <td><%=list.get(j).getGoods_id()%></td>
                        <td><%=list.get(j).getKind_id()%></td>
                        <td><%=list.get(j).getPrice()%></td>
                        <td><%=list.get(j).getSales_number()%></td>
                        <td><%=list.get(j).getStock_number()%></td>
                        <td><%=list.get(j).getInfo()%></td>
                        <td><%=list.get(j).getFileadress()%></td>
                        <td>
                            <a href="javascript:UpdateGoods(<%=list.get(j).getGoods_id()%>)" >编辑</a>
                            <a href="javascript:del('<%=list.get(j).getGoods_id()%>');">删除</a>
                        </td>
                    </tr>
                <%
                     }
    ...
    <%--表格底--%>
            ...
            <div class="ui_tb_h30">
                <div class="ui_flt" style="height: 30px; line-height: 30px;">
                    ...
                    条记录，当前第
                    <span class="ui_txt_bold04"><%=page_num%>
						/
						<%=a%></span>
                    页
                </div>
